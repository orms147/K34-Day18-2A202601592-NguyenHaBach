...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        sentences = [s.strip() for s in re.split(r'(?<=[.!?])\s+', parent_text) if s.strip()]
        current_child_text = ""
    ...
    for section in sections:
        if re.match(r'^#{1,3}\s+.+$', section):
            if current_content.strip():
                ...
                chunks.append(Chunk(text=text_to_save.strip(), metadata={**metadata, "section": current_header.strip(), "strategy": "structure"}))
